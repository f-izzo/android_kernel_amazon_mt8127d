...
{
    //printk(KERN_ERR"gic_irq (irq=%d, hwirq=%ld)\n", d->irq, d->hwirq);
	return d->irq;
}

static int mt_set_affinity(struct irq_data *d, const struct cpumask *mask_val,
							bool force)
{
	void __iomem *reg = GIC_DIST_BASE + GIC_DIST_TARGET + (gic_irq(d) & ~3);
    ...
    //printk(KERN_ERR"mt_set_affinity enter(nr_cpu_ids=%d, cpu==%d, shift==%d)\n", nr_cpu_ids, cpu, shift);
	...
	val = readl(reg) & ~mask;
    //printk(KERN_ERR"mt_set_affinity (val=%d, bit=%d, reg=%p)\n", val, bit, reg);
	writel(val | bit, IOMEM(reg));
...
static struct irq_chip mt_irq_chip = {
	...
	.irq_set_affinity	= mt_set_affinity,
};
...
static void mt_gic_cpu_init(void)
{
	...
	int i;
    printk(KERN_ERR"mt_gic_cpu_init enter(cpu==%d)\n", cpu);
	/*
    ...
    printk(KERN_ERR"gic_cpu_map[%d]==%d\n", cpu, gic_cpu_map[cpu]);
