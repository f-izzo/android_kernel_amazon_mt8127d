...
int sym827_read_byte(kal_uint8 cmd, kal_uint8 *data)
{
	...
	I2C_WR_FLAG | I2C_RS_FLAG | I2C_DIRECTION_FLAG;
...
void sym827_hw_component_detect(void)
{
	...
	ret_rev = sym827_read_interface(SYM827_REG_ID_2, &val_rev, 0x0F, 0x0);
...
int sym827_buck_set_voltage(unsigned long voltage)
{
	...
	*/
	if (voltage > 1300000) {
		pr_err("too high sym827 voltage = %lu, reg = %d ,vol_sel = %x, get = %x\n",
	...
	if (vol_sel >= 56) { // 56 maps to 1.3v, which is the highest that MT8127 can take
		pr_err("sym827 voltage calcuation result too high, voltage= %lu, vol_sel=%x, resetting to 56",
			voltage, vol_sel);
		vol_sel = 56;
	}
				   ...
				   struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		if (size > 4) {
			ret = kstrtoul(strsep(&buf, " "), 16, &reg_address);
			if (ret)
				return ret;
		...
		pr_err("reg_address: %x, reg_value: %x\n", reg_address, reg_value);
	}
	return size;
}
...
static int sym827_driver_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	if (!new_client) {
		err = -ENOMEM;
		dev_err(&i2c->dev, "[sym827_driver_probe] ENOMEM err\n");
		goto exit;
	}
