...
{
    int ret = 0;
	vsync_config_t vsync_timestamp;
    ...
    switch (cmd)
    {
        case MTKFB_VSYNC_IOCTL:
        {
			...
			/* For tank, there is only HDMI */
#if MTK_DISP_DIRECTLINK
            mtkfb_waitVsync();
#else
			hdmi_waitVsync();
#endif
		vsync_timestamp.vsync_ticks = ktime_to_ns(vsync_time);
                pr_debug("Pawan hdmi vsync interval %llu\n",vsync_timestamp.vsync_ticks);
		ret = copy_to_user(argp, &vsync_timestamp, sizeof(vsync_timestamp));
		if (ret)
			pr_err("vsync copy_to_user timestamp fail\n");
