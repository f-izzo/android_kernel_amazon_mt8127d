...
static int32_t cmdq_sec_setup_context_session(cmdqSecContextHandle handle)
{
	...
	if(NULL == handle->iwcMessage)
	{
		CMDQ_ERR("iwcMessage kmalloc failed!\n");
		return -ENOMEM;
	}
            ...
            CmdqSecFillIwcCB iwcFillCB )
{
    ...
    if(NULL != pTask && CMDQ_INVALID_THREAD != thread)
    {
        ...
        // metadata
        CMDQ_MSG("[SEC]SESSION_MSG: addrList[%d][0x%p], portList[%d][0x%p] srcHandleForHDCP[0x%x] dstHandleForHDCP[0x%x]\n",
            pTask->secData.addrListLength,
            pTask->secData.addrList,
            pTask->secData.portListLength,
            pTask->secData.portList,
            pTask->secData.srcHandleForHDCP,
            pTask->secData.dstHandleForHDCP);
		...
		pIwc->metadata.srcHandleForHDCP = pTask->secData.srcHandleForHDCP;
		pIwc->metadata.dstHandleForHDCP = pTask->secData.dstHandleForHDCP;
            ...
            CmdqSecFillIwcCB iwcFillCB)
{
	...
	CMDQ_MSG("submit to secure world srcHandleForHDCP[0x%x] dstHandleForHDCP[0x%x]\n",
		pTask->secData.srcHandleForHDCP, pTask->secData.dstHandleForHDCP);
...
cmdqSecContextHandle cmdq_sec_context_handle_create(uint32_t tgid)
{
    cmdqSecContextHandle handle = NULL;
#ifdef CONFIG_CMDQ_DMAPOOL_SUPPORT	
	handle = cmdq_alloc_dma_pool(sizeof(cmdqSecContextStruct), NULL);
#else
