...
#define BTDAI_MODEM_INTF /*Support BT-SCO via externa modem pcm interface, only verified Rx path 20160429*/
#ifdef DEBUG_AUDDRV
#define PRINTK_AUDDRV(format, args...) pr_err(format, ##args)
#else
...
#ifdef DEBUG_AUD_CLK
#define PRINTK_AUD_CLK(format, args...)  pr_err(format, ##args)
#else
...
#define PRINTK_AUD_HDMI(format, args...)  pr_err(format, ##args)
#else
