...
/*****************************************************************************
 *fe_reg_remap(                E X T E R N A L   R E F E R E N C E S
 *****************************************************************************/
...
int mt_afe_reg_remap(void *dev)
{
 ...
 exit:
	...
	spm_base_address = (phys_addr_t *)SPM_BASE;
	apmixedsys_base_address = (phys_addr_t *)APMIXEDSYS_BASE;
#else
	spm_base_address = ioremap_nocache(SPM_BASE, 0x1000);
	apmixedsys_base_address = ioremap_nocache(APMIXEDSYS_BASE, 0x1000);
#endif
	PRINTK_AUDDRV("afe_sram_address 0x%p\n", (phys_addr_t *)afe_sram_address);
	PRINTK_AUDDRV("afe_base_address 0x%p\n", (phys_addr_t *)afe_base_address);
	PRINTK_AUDDRV("topckgen_base_address 0x%p\n", (phys_addr_t *)topckgen_base_address);
	PRINTK_AUDDRV("spm_base_address 0x%p\n", (phys_addr_t *)spm_base_address);
	PRINTK_AUDDRV("apmixedsys_base_address 0x%p\n", (phys_addr_t *)apmixedsys_base_address);
	return 0;
#endif
}
...
void mt_afe_set_reg(uint32_t offset, uint32_t value, uint32_t mask)/*Afe_Set_Reg*/
{
	...
	/*pr_err("%s afe_base_address %p\n", __func__, afe_base_address);*/
...
uint32_t GetClkCfg(uint32_t offset)
{
	...
	if(offset > AUDIO_CLKCFG_MAX_OFFSET) {
		return 0xFFFFFFFF;
	}
