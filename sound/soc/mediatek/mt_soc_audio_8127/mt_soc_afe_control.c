...
irqreturn_t mt_afe_irq_handler(int irq, void *dev_id)/*AudDrv_IRQ_handler*/
{
	...
	if (u4RegValue & INTERRUPT_IRQ2_MCU) {
		...
		if (audio_mem_if[Soc_Aud_Digital_Block_MEM_MOD_DAI]->state == true)
			afe_modem_dai_interrupt_handler();
	}
	...
	if (direction) {
		...
		case Soc_Aud_InterConnectionInput_I09:
			if (Soc_Aud_PCM_MODE_PCM_MODE_8K ==
				...
				mt_afe_set_reg(AFE_SGEN_CON0, 0x540e80e8, 0xffffffff);
			else if (Soc_Aud_PCM_MODE_PCM_MODE_16K ==
				 audio_mem_if[Soc_Aud_Digital_Block_MEM_MOD_DAI]->sample_rate)
				mt_afe_set_reg(AFE_SGEN_CON0, 0x543e43e4, 0xffffffff);
			break;
...
/*		case Soc_Aud_InterConnectionInput_I12:
		...
		case Soc_Aud_InterConnectionOutput_O08:
			if (Soc_Aud_PCM_MODE_PCM_MODE_8K ==
			    audio_mem_if[Soc_Aud_Digital_Block_MEM_MOD_DAI]->sample_rate)
				mt_afe_set_reg(AFE_SGEN_CON0, 0xb40e80e8, 0xffffffff);
			else if (Soc_Aud_PCM_MODE_PCM_MODE_16K ==
				 audio_mem_if[Soc_Aud_Digital_Block_MEM_MOD_DAI]->sample_rate)
				mt_afe_set_reg(AFE_SGEN_CON0, 0xb43e43e4, 0xffffffff);
			break;
...
bool SetModemPcmConfig(int modem_index, struct AudioDigitalPCM p_modem_pcm_attribute)
{
	...
	if (modem_index == MODEM_1) {
		...
		if (p_modem_pcm_attribute.mPcmModeWidebandSel == Soc_Aud_PCM_MODE_PCM_MODE_8K) {
			...
			audio_mem_if[Soc_Aud_Digital_Block_MODEM_PCM_2_O]->sample_rate =
		...
		reg_pcm_intf_con1 |= (p_modem_pcm_attribute.mSyncOutInv & 0x01) << 23;
		reg_pcm_intf_con1 |= (p_modem_pcm_attribute.mBclkOutInv & 0x01) << 22;
		reg_pcm_intf_con1 |= (p_modem_pcm_attribute.mSyncInInv & 0x01) << 21;
...
bool SetModemPcmEnable(int modem_index, bool modem_pcm_on)
{
	...
	if (modem_index == MODEM_1) {	/* MODEM_1 use PCM2_INTF_CON (0x53C) */
		...
		if (modem_pcm_on == true) {	/* turn on ASRC before Modem PCM on */
			...
			if (mPcm1AsyncFifo == 0)
				mt_afe_set_reg(AFE_ASRC_CON0, 0x06003031, MASK_ALL);
...
void afe_modem_dai_interrupt_handler(void)
{
	...
	if (Mem_Block == NULL)
		return;
	spin_lock_irqsave(&Mem_Block->substream_lock, flags);
	if (mt_afe_get_memory_path_state(Soc_Aud_Digital_Block_MEM_MOD_DAI) == false) {
		pr_err("%s, mt_afe_get_memory_path_state(Soc_Aud_Digital_Block_MEM_MOD_DAI) == false\n",
		     __func__);
		spin_unlock_irqrestore(&Mem_Block->substream_lock, flags);
		return;
	}
