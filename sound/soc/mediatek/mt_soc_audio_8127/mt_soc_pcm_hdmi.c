...
static void mtk_asoc_pcm_hdmi_free(struct snd_pcm *pcm);
...
static const char *const hdmi_format_change[] = { "Off", "On" };

static int hdmi_format_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static int hdmi_format_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_platform *platform = snd_kcontrol_chip(kcontrol);
	struct mt_pcm_hdmi_priv *priv = snd_soc_platform_get_drvdata(platform);
...
static const struct soc_enum mt_pcm_hdmi_control_enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(hdmi_format_change), hdmi_format_change),
};
...
static const struct snd_kcontrol_new Audio_snd_hdmi_controls[] = {
	...
	SOC_ENUM_EXT("Audio_Hdmi_SideGen_Switch", mt_pcm_hdmi_control_enum[0], Audio_hdmi_SideGen_Get,
		     Audio_hdmi_SideGen_Set),
	SOC_ENUM_EXT("HDMI_Force_Clk_Switch", mt_pcm_hdmi_control_enum[1], hdmi_force_clk_get,
		     hdmi_force_clk_set),
	SOC_ENUM_EXT("Audio_Hdmi_Format_Switch", mt_pcm_hdmi_control_enum[2], hdmi_format_get,
		     hdmi_format_set),
};
...
static int mtk_pcm_hdmi_prepare(struct snd_pcm_substream *substream)
{
	...
	(priv->cached_channels != runtime->channels) || priv->format_changed) {
		if (priv->is_apll_related_clks_on) {
			mt_afe_hdmi_clk_off();
			mt_afe_aplltuner_clk_off();
			priv->is_apll_related_clks_on = false;
...
static int mtk_pcm_hdmi_start(struct snd_pcm_substream *substream)
{
	...
	PRINTK_AUD_HDMI("%s period_size = %lu,runtime->rate=%u,runtime->channels=%u\n",
...
static struct snd_soc_platform_driver mtk_hdmi_soc_platform = {
	...
	.pcm_free = mtk_asoc_pcm_hdmi_free,
...
static int mtk_asoc_pcm_hdmi_new(struct snd_soc_pcm_runtime *rtd)
{
	...
	size = mtk_hdmi_hardware.buffer_bytes_max;

	ret = snd_pcm_lib_preallocate_pages_for_all(pcm, SNDRV_DMA_TYPE_DEV,
			card->dev, size, size);
	if (!ret)
		PRINTK_AUD_HDMI("%s preallocate successful\n", __func__);
	else
